...
generate_mc <- function(name,
                        ...
                        mc_suf = "",
                        force_cgraph = NULL) {
    if (is.null(.scfigs_base)) {
        scfigs_init("figs/")
    }
    mat_nm <- name
    mc_nm <- sprintf("%s_bs500%s", name, mc_suf)
    mcf_nm <- sprintf("%s_bs500f%s", name, mc_suf)
    coc_nm <- sprintf("%s_500", name)
    graph_nm <- mat_nm
    if (!is.null(force_cgraph)) {
        graph_nm <- force_cgraph
    }

    db_fn <- .scdb_base
    should_recomp <- recompute |
        !file.exists(sprintf("%s/mc.%s.Rda", db_fn, mcf_nm))
    if (should_recomp) {
        if (is.null(force_cgraph)) {
        ...
        mcell_coclust_from_graph_resamp(coc_nm, graph_nm,
            min_mc_size = 15, p_resamp = 0.75, n_resamp = 500
        )
        ...
        mcell_mc_split_filt(new_mc_id = mcf_nm, mc_nm, mat_nm, T_lfc = T_lfc, plot_mats = F)
    }
