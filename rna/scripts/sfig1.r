...
gen_sfig1 <- function() {
    ...
    pheatmap::pheatmap(
        pmin(pmax(bad_cors[order(km$cluster), order(km$cluster)], -0.3), 0.3),
        cluster_cols = F, cluster_rows = F, color = shades, filename = "paper_figs/sfig1/bad_genes_wt_cors.png", width = 25, heigh = 25, fonsize = 3
    )
