...
if (do_fig2) {
    ebdnmt_plot_kit(
        mc_id = "eseb_07_3ab_bs500f_got_s2m",
        mat_id = "eseb_07_3ab",
    ...
    ebdnmt_plot_kit(
        mc_id = "eseb_07_TKO_bs500f_got_s2m",
        mat_id = "eseb_07_TKO",
    ...
    for (g in c("Dnmt3l", "Tet1", "Lefty1", "Lefty2", "Nodal", "Ifitm1", "Ifitm3", "Dppa3")) {
        # restrict to j1 wt
        plot_g_gmod_compare(d,
            gmods = gmods, gmod = "epi", gene = g,
    ...
    for (g in c("Lefty1", "Lefty2", "Nodal", "Cer1", "Cfc1", "Tdgf1", "Eomes", "Tal1", "Foxa2", "Mesp1")) {
        ...
        plot(log2(dko_l$strati1_n[g, ] + 1e-5),
            type = "l", col = "darkblue",
...
if (do_fig3) {
    ebdnmt_plot_kit(
        mc_id = "eseb_07_3a_bs500f_got_s2m",
        mat_id = "eseb_07_3a",
    ...
    ebdnmt_plot_kit(
        mc_id = "eseb_07_3b_bs500f_got_s2m",
        mat_id = "eseb_07_3b",
...
plot_ab_rna_compare <- function() {
    ...
    mc_t_e <- apply(mc_t, 1, function(x) {
        return(sum(x * (1:max_t)) / sum(x))
    })
    f <- mc_t_e > 3
    ...
    mc_t_e <- apply(mc_t, 1, function(x) {
        return(sum(x * (0:max_t)) / sum(x))
    })
    f_eb <- mc_t_e > 3

    png("paper_figs/fig3/rna_3a3b.png", w = 1000, h = 500)
    layout(matrix(1:2, nrow = 1))
    plot(iv_legc["Dnmt3a", f], iv_legc["Dnmt3b", f],
        pch = 21, bg = mc_layer_col[f],
        xlim = c(-17, -10), ylim = c(-15, -7.5), cex = 1.5
    )
    plot(eb_legc["Dnmt3a", f_eb], eb_legc["Dnmt3b", f_eb],
        pch = 21,
