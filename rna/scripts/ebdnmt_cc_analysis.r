    ...
    for (col in cols) {
        ...
        for (t in 1:7) {
            ...
            if (length(cells) > 100) {
                ...
                df <- data.frame(
                    mki67 = c_mki67, pcna = c_pcna, lab = names(c_mki67),
    ...
    for (col in cols) {
        ...
        for (t in 1:7) {
            ...
            if (length(cells) > 100) {
                ...
                fmean <- function(x) {
                    return(ifelse(length(x) > 10, mean(x), NA))
                }
...
plot_mm_cc <- function(tab, layer = NA, d, cg = NA, foc_line = NA, ylim = NULL, foc_cells = NULL) {
    ...
    par(mar = c(2, 1, 0, 2))
    plot(tab$ord2, tab$avg_early,
        pch = 19, col = "darkblue",
        ylim = c(
            min(c(tab$avg_early, tab$avg_late)),
...
plot_mm_ab_dlt <- function(tab, layer = NA, d, cg = NA, foc_line = NA, ylim = NULL, foc_cells = NULL) {
    ...
    tab_merg <- tab_low_ab %>%
        select(cell_id, cg_cont, day, ord1, early_late_cov, lab_late, lab_early) %>%
        left_join(tab_high_ab %>% select(cell_id, cg_cont, day, hab_late, hab_early))
    ...
    if (nrow(tab) > 50) {
        ...
        plot(tab$ord2, tab$hab_early - tab$lab_early,
        ...
        boxplot(split(tab$d_early, cut(tab$ord2, seq(0, 1, l = 10))),
            add = T,
            at = seq(0.02, 0.92, l = 9), boxwex = 0.02, col = "darkblue"
        )
        boxplot(split(tab$d_late, cut(tab$ord2, seq(0, 1, l = 10))),
            add = T,
