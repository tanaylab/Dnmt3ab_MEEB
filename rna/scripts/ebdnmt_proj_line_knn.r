...
proj_knn_line <- function(ref_mat_id, ref_mc_id, q_mat_id, q_mc_id, gset_id) {
    ...
    q_feats <- gset_get_feat_mat(
        gset_id = gset_id, mat_id = q_mat_id,
        downsamp = T, add_non_dsamp = T,
        downsample_n = dsamp_n
    )

    a_feats <- gset_get_feat_mat(
        gset_id = gset_id, mat_id = ref_mat_id,
        downsamp = T, add_non_dsamp = T,
        downsample_n = dsamp_n
    )
    ...
    gr <- tgs_cor_knn(
        log2(1 + k_nonz_exp * as.matrix(q_feats)),
    ...
    tot_j1_q <- t(tgs_matrix_tapply(
        qmat@mat[, match_j1$col1],
    ...
    tot_n15_q <- t(tgs_matrix_tapply(
        qmat@mat[, match_n15$col1],
...
bxplt_cmp <- function(gnm, res1, res2, qmc1, qmc2) {
    ...
    n15_splt <- split(
        res1$n15_legc[gnm, n15_mcs] - res1$n15_p_legc[gnm, n15_mcs],
        qmc1@colors[n15_mcs]
    )
    boxplot(n15_splt, col = names(n15_splt))
    j1_splt <- split(
        res1$j1_legc[gnm, j1_mcs] - res1$j1_p_legc[gnm, j1_mcs],
        qmc1@colors[j1_mcs]
    )
    ...
    n15_splt <- split(
        res2$n15_legc[gnm, n15_mcs] - res2$n15_p_legc[gnm, n15_mcs],
        qmc1@colors[n15_mcs]
    )
    boxplot(n15_splt, col = names(n15_splt))
    j1_splt <- split(
        res2$j1_legc[gnm, j1_mcs] - res2$j1_p_legc[gnm, j1_mcs],
        qmc1@colors[j1_mcs]
    )
    boxplot(j1_splt, col = names(j1_splt))
}
