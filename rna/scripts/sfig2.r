gen_sfig2_atlas <- function() {
    ...
    mcell_mc_plot_marks("emb_gotg_75_bs500f", "eseb_key_tfs",
        mat = "emb_gotg_75",
    ...
    segments(
        mc2d@mc_x[mc2d@graph$mc1], mc2d@mc_y[mc2d@graph$mc1],
...
gen_sfig2_cmp_proj_and_epi <- function() {
    ...
    head(mc_d)
    mc_d_e <- apply(mc_d, 1, function(x) {
        return(sum(x * (0:7)) / sum(x))
    })
