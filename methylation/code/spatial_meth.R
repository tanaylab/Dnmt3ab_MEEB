plot_spatial_meth <- function(intervals, tracks, expand_s=0, expand_e=0, min_cov=10, names=NULL, md=NULL, k=NULL, scope=5e5, separate_plots=FALSE){
    ...
    if (!is.null(k)){
        d <- d %>% group_by(samp) %>% arrange(samp, start) %>% mutate(meth_s = zoo::rollsum(meth, k=k, fill="extend"), cov_s = zoo::rollsum(cov, k=k, fill="extend")) %>% mutate(avg_s = meth_s / cov_s) %>% select(chrom:end, cov=cov_s, samp, avg=avg_s)
    } 

    d1 <- d %>% filter(cov >= min_cov) %>% select(chrom:end, samp, avg)
    if (!is.null(md)){
        d1 <- d1 %>% left_join(md)
    }
    p_meth <- d1 %>% ggplot(aes(x=start, y=avg, color=samp, group=samp)) + geom_point() + geom_line() + scale_x_continuous(limits=c(expanded_intervals$start, expanded_intervals$end)) + geom_vline(xintercept=c(intervals$start, intervals$end), color="gray", linetype="dashed") + ggsci::scale_color_lancet(name="") + ylab("Avg. Meth.") + xlab("")
    ...
    genes <- gintervals.neighbors1("intervs.global.tss", scope_intervals) %>% filter(dist == 0) %>% distinct(geneSymbol, .keep_all = TRUE) %>% select(chrom, start, end, strand, name=geneSymbol)
...
get_smooth_meth_data <- function(tracks, names=NULL, min_cov = 20, d_expand = 100, hits_iterator = d_expand / 2, min_samples=NULL, screen = TRUE, intervals = NULL){
    ...
    hits_data <- gextract(c(glue("{meth_vtracks} / {cov_vtracks}"), cov_vtracks), iterator=hits_mid, intervals=intervals, colnames=c(names, glue("{names}.cov"))) %>% arrange(intervalID) %>% select(-intervalID)  %>% as_tibble()    
