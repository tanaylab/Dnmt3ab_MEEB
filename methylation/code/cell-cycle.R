...
get_cc_segments <- function(df, n_breaks = 2, psi=c(0.2, 0.5), labels=c("S-start", "S-mid", "S-end"), ...){
    ...
    trend_df_cov <- df_wrap %>% mutate(cov_trend = predict(cov_model) ) %>% filter(ord2 >= 1, ord2 <= 2) %>% mutate(ord2 = ord2 - 1)
...
plot_cc_early_late_meth <- function(df, point_size = 0.5, add_trend_lines = FALSE, phases=NULL, plot_phase_lines = FALSE, y_lim=c(0.65, 1), rm_legend = TRUE, trend_ylim = NULL){
    ...
    if (add_trend_lines){
        ...
        p_early_late_cov <- df %>% filter(!is.na(ord2)) %>% ggplot(aes(x=ord2, y=early_late_cov)) + geom_point(size = point_size, alpha = alpha)
    }
...
plot_cell_cycle_ord_diagnostics <- function(l, title=NULL){    
    ...
    print(p_curve)
    p_early_late_cov <- df %>% filter(!is.na(ord)) %>% ggplot(aes(x=ord, y=early_late_cov)) + geom_point()
    p_early_late_avg <- df %>% select(cell_id, early=avg_early, late=avg_late, ord) %>% gather("type", "meth", -cell_id, -ord) %>% ggplot(aes(x=ord, y=meth, color=type)) + geom_point(size=0.5) + ggsci::scale_color_lancet(name="")    
...
get_S_cpgs <- function(segmented = fread(here("output/cell_cycle/segmented.tsv")), early=TRUE, min_cov = 10, min_cov_bulk = 20, min_meth_bulk = 0.7){
    ...
    s_cpgs_raw <- db_f %>% left_join_cells(segmented_wt %>% select(cell_id, phase) %>% filter(phase != "other")) %>% filter_cells(!is.na(phase)) %>% group_by_cells(phase) %>% summarise()

    if (early){
        s_cpgs <- s_cpgs_raw %>% filter(tor > 0, cov >= min_cov) %>% mutate(avg = meth / cov) %>% pivot_wider(names_from="phase", values_from=c("meth", "cov", "avg"))     
    } else {
        s_cpgs <- s_cpgs_raw %>% filter(tor < 0, cov >= min_cov) %>% mutate(avg = meth / cov) %>% pivot_wider(names_from="phase", values_from=c("meth", "cov", "avg")) 
    }    
    ...
    s_cpgs_f <- s_cpgs %>% left_join(bulk_meth %>% select(chrom:end, ecto, meso, endo) ) %>% filter(ecto >= min_meth_bulk)    
