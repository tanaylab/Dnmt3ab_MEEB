...
library(misha.ext)
library(misha.vis)
# devtools::load_all("/home/aviezerl/repo/misha.vis", warn_conflicts = FALSE, export_all = FALSE)
# devtools::load_all("/home/aviezerl/repo/sc5mc", warn_conflicts = FALSE)
...
tracks_key <- tracks_key %>% group_by(day, line, sort, capture, group)  %>% mutate(replicate = 1:n())  %>% mutate(name = glue("{day}_{line}_{sort}_{capture}_r{replicate}_{group}"), name = gsub("\\.", "_", name), name = gsub("/", "_", name), name = gsub("\\?", "", name)) %>% ungroup() 

track_stats <- calc_track_stats() %cache_df% here("output/track_stats.csv") %>% filter(CHH <= 0.02)
tracks_key <- tracks_key %>% filter(track_name %in% track_stats$track)

ext_tracks_key <- fread(here("data/ext_tracks_key.tsv")) %>% mutate(name =  glue("{day}_{line}_{sort}_{capture}_r{replicate}_{group}")) %>% mutate(sort = ifelse(sort == "Epi", "epi", sort)) %>% mutate(sort_fraction = NA)
chip_tracks_key <- fread(here("data/chip_tracks.tsv")) 


meth_md <- tracks_key %>% mutate(meth_track = glue("{track_name}.meth"), cov_track = glue("{track_name}.cov"), meth_vtrack = glue("{name}.meth"), cov_vtrack = glue("{name}.cov")) %>% mutate(expr = glue("{meth_vtrack} / {cov_vtrack}"))

ext_meth_md <- ext_tracks_key %>% mutate(meth_track = glue("{track_name}.meth"), cov_track = glue("{track_name}.cov"), meth_vtrack = glue("{name}.meth"), cov_vtrack = glue("{name}.cov")) %>% mutate(expr = glue("{meth_vtrack} / {cov_vtrack}"))
