...
calc_on_target_stats <- function(){    
    ...
    stats <- bind_rows(V1_stats, V2_stats, wg_stats) %>% organize_samp_id() %>% as_tibble()
    return(stats)
}

organize_samp_id <- function(df){
    df <- df %>% left_join(tracks_key %>% select(track = track_name, mut=line, day) %>% unite("samp_id", day, mut, remove=TRUE)) %>% 
        mutate(line = ifelse(grepl("N15", samp_id), "N15", "J1")) %>% 
        mutate(samp_id = gsub(".cov$", "", samp_id), 
...
organize_sc_samp_id <- function(df){
    df <- df %>% unite("samp_id", day, line, remove = FALSE) %>% 
        mutate(
            samp_id = gsub("_ko3a", "_3a", samp_id),
            samp_id = gsub("_ko3b", "_3b", samp_id),
...
calc_track_stats <- function(tracks = fread(here("data/tracks_key.tsv"))$track_name) {    
    ...
    stats <- stats %>% organize_samp_id() %>%  as_tibble()
    return(stats)
}
...
add_enh_cov <- function(fn = here("data/tracks_key.tsv")){
    ...
    get_track_enh_cov <- function(x){
        if (gtrack.exists(glue("{x}.cov"))){
            ...
            res <- gextract(glue("vt_{x}.cov"), intervals = enh_intervs, iterator = enh_intervs, colnames="cov") %>% replace_na(replace = list(cov = 0)) %>% summarise(tot_cov = sum(cov), mean_cov = mean(cov, na.rm=TRUE))
        } else {
