...
meth_dip_screen <- function(track, intervals = gintervals.all(), iterator = "intervs.global.seq_CG", dip_size = 300, flank_size = 500, margin = 50, min_diff = 0.5, min_cov = 10, min_nhood_cov = min_cov, canonize = FALSE, ret_values = FALSE) {
    ...
    if (ret_values) {
        df <- gextract(c(center_expr, left_expr, right_expr, "cov_vt_center", "cov_vt_left", "cov_vt_right", "meth_vt_center", "meth_vt_left", "meth_vt_right"), iterator = df %>% select(chrom, start, end), intervals = df %>% select(chrom, start, end), colnames = c("center", "left", "right", "center.cov", "left.cov", "right.cov", "center.meth", "left.meth", "right.meth")) %>%
            arrange(intervalID) %>%
            select(-intervalID) %>%
            as_tibble()
        df <- df %>%
